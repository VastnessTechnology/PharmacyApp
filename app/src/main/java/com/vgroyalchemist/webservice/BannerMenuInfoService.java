...
import com.vgroyalchemist.requestobjects.BannerMenuListInfo;
...
public class BannerMenuInfoService {

    public JSONObject generateRequestJSON(String MenuName) {

        JSONObject requestObject = null;
        JSONObject mObject = null;
        RequestFactory mRequestFactory = new RequestFactory();
        BannerMenuListInfo listInfo = new BannerMenuListInfo(MenuName);
        try {
    ...
    public ServerResponseVO bannermenu(Context context, String url ,String MenuName) {
        ServerResponseVO serverResponseVO = new ServerResponseVO();

        try {
            JSONObject requestJSON = generateRequestJSON(MenuName);

            JSONObject responseJSON = sendRequest(url, requestJSON, context);
            Utility.debugger("VT Banner ....." + url);
            Utility.debugger("VT Banner Request....." + requestJSON);
            Utility.debugger("VT responseJSON  Banner..... " + responseJSON);
            ...
            try {
                ...
                for (int m = 0; m < jsonArray.length(); m++) {
                    ...
                    try {
                        getAllBannerMenuVO.setBanner(mJsonObject.getString("Banner"));
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
